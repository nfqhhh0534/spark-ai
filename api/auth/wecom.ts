...
const WECOM_AGENT_ID = process.env.WECOM_AGENT_ID || '1000355';
...
// 通过 code 获取用户信息
async function getUserInfoByCode(code: string, accessToken: string): Promise<WeComUserInfo | null> {
  try {
    const url = `https://qyapi.weixin.qq.com/cgi-bin/user/getuserinfo?access_token=${accessToken}&code=${code}`;
    const response = await fetch(url);
    const data: WeComResponse & { userid?: string } = await response.json();
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // 通过 code 获取用户信息
    const userInfo = await getUserInfoByCode(code, accessToken);
    if (!userInfo) {
