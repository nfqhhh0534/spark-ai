...
import { currentUser, questions, projects, insights, products, tags } from '@/data/mockData';
    ...
    const initDb = async () => {
      try {
        ...
        if (!existingProfile) {
          ...
          if (insertError) {
            console.warn('⚠️ Profile insert error:', insertError);
          } else {
            console.log('✅ User profile created in database');
          }
        }

        // Try to fetch questions from Supabase
        ...
        if (dbQuestions && dbQuestions.length > 0) {
          ...
          dispatch({ type: 'SET_DB_CONNECTED', payload: true });
          console.log('📦 Using mock data (database is empty or query returned null)');
        }
